...
public class LandForms {
    ...
    public Module steppe(Seed seed) {
        ...
        NoiseSource warpX = Source.build(seed.next(), scaleH / 4, 3).lacunarity(3.0).perlin();
        NoiseSource warpY = Source.build(seed.next(), scaleH / 4, 3).lacunarity(3.0).perlin();
        Module module = Source.perlin(seed.next(), scaleH, 1).mult(erosion).warp(warpX, warpY, Source.constant((float)scaleH / 4.0f)).warp(seed.next(), 256, 1, 200.0);
    ...
    public Module plains(Seed seed) {
        ...
        double erosionAmount = 0.45;
        Module erosion = Source.build(seed.next(), scaleH * 2, 3).lacunarity(3.75).perlin().alpha(erosionAmount);
        NoiseSource warpX = Source.build(seed.next(), scaleH / 4, 3).lacunarity(3.5).perlin();
        NoiseSource warpY = Source.build(seed.next(), scaleH / 4, 3).lacunarity(3.5).perlin();
        Module module = Source.perlin(seed.next(), scaleH, 1).mult(erosion).warp(warpX, warpY, Source.constant((float)scaleH / 4.0f)).warp(seed.next(), 256, 1, 256.0);
    ...
    public Module hills1(Seed seed) {
        return Source.perlin(seed.next(), 200, 3).mult(Source.billow(seed.next(), MOUNTAINS2_H, 3).alpha(0.5)).warp(seed.next(), 30, 3, 20.0).warp(seed.next(), MOUNTAINS2_H, 3, 200.0).scale(0.6f * this.terrainVerticalScale);
    }

    public Module hills2(Seed seed) {
        return Source.cubic(seed.next(), 128, 2).mult(Source.perlin(seed.next(), 32, 4).alpha(0.075)).warp(seed.next(), 30, 3, 20.0).warp(seed.next(), MOUNTAINS2_H, 3, 200.0).mult(Source.ridge(seed.next(), 512, 2).alpha(0.8)).scale(0.55f * this.terrainVerticalScale);
    }
    ...
    public Module badlands(Seed seed) {
        ...
        Module hills = Source.ridge(seed.next(), 275, 4).warp(seed.next(), MOUNTAINS2_H, 2, 100.0).warp(seed.next(), 18, 1, 20.0).mult(mask);
