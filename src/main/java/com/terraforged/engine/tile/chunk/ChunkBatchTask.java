...
public class ChunkBatchTask implements BatchTask {
    ...
    public ChunkBatchTask(int seed, int x, int z, int size, Tile tile, Heightmap heightmap) {
    	this.seed = seed;
    ...
    private void drive() {
        ...
        int maxZ = Math.min(this.tile.getChunkSize().total, this.z + this.size);
        for (int cz = this.z; cz < maxZ; ++cz) {
            for (int cx = this.x; cx < maxX; ++cx) {
                this.driveOne(this.seed, this.tile.getChunkWriter(cx, cz), this.heightmap);
            }
        }
    }
